...
def watermark():
    print("222 bilgi hack topluluğuna armagan by @weertyyyy :)")

def get_number():
    global clear
    while True:
        try:
            number = int(input(f"""Telefon numarasını yaz caz yapma. Şunun gibi kokoş: "54xxxxxxxx" (Sadece Türkiye zorunamı giti tr dedik sorgulama!)\n[okuma yaz aq] : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"Numara Yanlış piç insan gibi gir ")
        except:
            clear()
            print(f"Lütfen bir numara yazın bak lütfen dedim yorma beni.")

def get_amount():
    global clear
    while True:
        try:
            amount = int(input(f"""Kaç SMS gönderim? Sınırsız gönderim için "0" basın.\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
